...
def twitters(request):
    ...
    if not embedded_tweets:
        ...
        try:
            ...
            tweets = api.request('statuses/user_timeline', {'count': 5})
            ...
            cache.set("twitters", embedded_tweets, 60 * 60)
        except:
            pass
...
def soundclouds(request):
    ...
    if not embedded_tracks:
        ...
        try:
            ...
            tracks = client.get('/users/' + sc['USER_ID'] + '/favorites', limit=5)
            for track in tracks:
...
def lastfms(request):
    ...
    if not lastfmtracks:
        ...
        try:
            ...
            url = "http://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=" + settings.USERNAME + "&format=json&limit=%d&api_key=%s" % (29, lastfmkey)
            ...
            for track in lastfmtracks:
                ...
                track.update({"artist": track['artist']['#text'], "date": date, "type": "lastfm"})
...
def flickrs(request):
    ...
    if not flickrs:
        ...
        try:
            ...
            all_photos = flickr.photos_search(user_id=f['USER_ID'], per_page='5')
            for photos in all_photos:
                for photo in photos:
                    ...
                    flickrs.append({"imgsrc": photolink, "photoid": p_id, "date": p_date, "type": "flickr", "description": description, "title": title})
...
def githubs(request):
    ...
    if not githubs:
        githubs = []
        try:
            me = github3.user(settings.USERNAME)
            ...
            for event in events:
                ...
                git_event.update({"date": date, "type": "github", "event_type": e_type})
    ...
    if not imgurs:
        ...
        try:
            ...
            for favorite in favorites:
                ...
                imgur = {"title": favorite['title'], "date": uploaded_at, "type": "imgur", "is_album": favorite['is_album']}
